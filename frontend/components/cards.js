...
export function renderHeaderStats(companies) {
  ...
  const tile = (label, value, extraClass = "") => `
    <div class="flex-1 min-w-[120px] rounded-lg border border-border bg-surface px-4 py-3">
      <div class="text-2xl font-mono font-semibold ${extraClass}">${value}</div>
  ...
  return `
    <div class="flex flex-wrap gap-3 mb-4">
      ...
      ${tile("On Track", counts.on_track, "text-good")}
      ${tile("Watch Closely", counts.watch_closely, "text-warn")}
      ${tile("Broken", counts.broken, "text-danger")}
      ${tile("Review Due", reviewDue, reviewDue ? "text-warn" : "")}
      ${tile("Active Overrides", activeOverrides, activeOverrides ? "text-danger" : "")}
    </div>`;
}

function metricChips(coreMetrics, metricDefsByKey) {
  const entries = Object.entries(coreMetrics || {}).slice(0, 4);
  if (!entries.length) return `<div class="text-xs text-muted-fg mt-2">No metrics yet</div>`;
  return `
      ...
      ${entries
        .map(([key, value]) => {
          ...
          return `<div class="text-xs">
            <span class="text-muted-fg">${escapeHtml(label)}</span>
            <span class="block font-mono font-medium text-fg">${formatMetricValue(value, unit, def?.decimals)}</span>
          </div>`;
        })
        .join("")}
    </div>`;
...
export function renderCards(companies, metricDefsByKey) {
  if (!companies.length) {
    return `<div class="col-span-full text-center text-muted-fg py-16">No companies match these filters.</div>`;
  }
  return companies
    .map((c) => {
      ...
      return `
      <button data-company-id="${escapeHtml(c.company_id)}"
        class="card-open text-left rounded-xl border border-border bg-surface p-4 hover:shadow-md transition-shadow relative">
        ${c.has_active_override ? `<span class="absolute top-3 right-3 text-[10px] font-semibold uppercase tracking-wide text-danger bg-danger/10 px-2 py-0.5 rounded-full ring-1 ring-danger/20">Override</span>` : ""}
        ...
        <div class="mt-2 font-semibold text-fg">${escapeHtml(c.name)}</div>
        <div class="text-xs text-muted-fg">${escapeHtml(c.broad_industry)} &gt; ${escapeHtml(c.specific_niche)}</div>
        <div class="text-xs text-muted-fg mt-1">${escapeHtml(c.operating_model)} · ${since === null ? "never reviewed" : `${since}d since review`}</div>

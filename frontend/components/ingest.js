...
export function renderIngestModalShell(taxonomy) {
  ...
  return `
    <div class="flex items-center justify-between px-5 py-3 border-b border-border">
      ...
      <button id="ingest-close" class="text-muted-fg hover:text-fg text-xl leading-none">&times;</button>
    </div>
    <div class="flex border-b border-border">
      <button id="ingest-tab-form" class="ingest-tab px-4 py-2 text-sm font-medium border-b-2 border-accent">Form</button>
      <button id="ingest-tab-json" class="ingest-tab px-4 py-2 text-sm font-medium border-b-2 border-transparent text-muted-fg">JSON</button>
    </div>
    <div class="p-5 overflow-y-auto" style="max-height: 70vh">
      ...
      <div id="ingest-form-panel">
        ...
        <label class="block text-sm mt-4">What It Does
          <textarea id="f-what-it-does" rows="2" class="mt-1 w-full rounded-md border border-border px-2 py-1.5 text-sm"></textarea>
        </label>
        ...
        <label class="block text-sm mt-4">The Growth Engine <span class="text-muted-fg">(one per line)</span>
          <textarea id="f-growth-engine" rows="2" class="mt-1 w-full rounded-md border border-border px-2 py-1.5 text-sm"></textarea>
        </label>

        <div class="grid grid-cols-2 gap-3 mt-3">
          <label class="text-sm">The Big Change - Summary
            <textarea id="f-big-change-summary" rows="2" class="mt-1 w-full rounded-md border border-border px-2 py-1.5 text-sm"></textarea>
          </label>
        ...
        <label class="block text-sm mt-3">Hard Evidence <span class="text-muted-fg">(one per line)</span>
          <textarea id="f-hard-evidence" rows="2" class="mt-1 w-full rounded-md border border-border px-2 py-1.5 text-sm"></textarea>
        </label>
        ...
        <div class="mt-4">
          <div class="text-sm font-medium">What Can Kill It <span class="text-muted-fg">(needs &ge; 1 severity=kill entry)</span></div>
        ...
        <label class="block text-sm mt-4">Why We Believe It <span class="text-muted-fg">(one per line, &ge;3 entries, start lines with "Premise" / "Inference" / "Conclusion" - exactly one Conclusion)</span>
          <textarea id="f-why-believe" rows="4" class="mt-1 w-full rounded-md border border-border px-2 py-1.5 text-sm"></textarea>
        </label>

        <label class="block text-sm mt-3">Latest Quarter Review
          <textarea id="f-latest-review" rows="2" class="mt-1 w-full rounded-md border border-border px-2 py-1.5 text-sm"></textarea>
        </label>
    ...
    <div class="px-5 py-3 border-t border-border flex justify-end gap-2">
      <button id="ingest-cancel" class="text-sm px-3 py-1.5 rounded-md border border-border hover:bg-surface-3">Cancel</button>
      <button id="ingest-submit" class="text-sm px-3 py-1.5 rounded-md bg-accent text-accent-ink hover:brightness-90">Create Company</button>
    </div>`;
}
